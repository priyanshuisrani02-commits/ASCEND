...
export default function UserProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  if (!profile) return <div className="min-h-screen bg-[#090a0f] text-slate-100 flex flex-col justify-between"><NavbarWrapper /><div className="max-w-7xl mx-auto px-4 py-20 text-center text-slate-400">{error || 'Loading competitor profile...'}</div><Footer /></div>;
  ...
  return <div className="min-h-screen bg-[#090a0f] text-slate-100 flex flex-col justify-between">
    ...
    <main className="flex-1 w-full pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"><div className="grid grid-cols-2 sm:grid-cols-4 gap-4"><div className="glass-panel p-5 rounded-2xl border border-slate-800"><div className="text-xs font-bold text-slate-400 uppercase tracking-wider">EARNED ACHIEVEMENTS</div><div className="text-2xl font-black text-white font-mono mt-1">{unlockedCount}</div></div><div className="glass-panel p-5 rounded-2xl border border-slate-800"><div className="text-xs font-bold text-blue-400 uppercase tracking-wider">RARE & EPIC</div><div className="text-2xl font-black text-blue-300 font-mono mt-1">{rareCount}</div></div><div className="glass-panel p-5 rounded-2xl border border-slate-800"><div className="text-xs font-bold text-amber-400 uppercase tracking-wider">VERIFIED RECORDS</div><div className="text-2xl font-black text-amber-300 font-mono mt-1">{records.length}</div></div><div className="glass-panel p-5 rounded-2xl border border-slate-800"><div className="text-xs font-bold text-emerald-400 uppercase tracking-wider">TOTAL XP</div><div className="text-2xl font-black text-emerald-300 font-mono mt-1">{profile.xp.toLocaleString()}</div></div></div></div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12"><div><h3 className="text-2xl font-black text-white mb-6">COMPETITIVE ACHIEVEMENTS</h3><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{achievements.map(ach => <AchievementCard key={ach.id} achievement={ach} />)}</div></div>{records.length > 0 && <div><h3 className="text-2xl font-black text-white mb-6">VERIFIED RECORDS</h3><div className="space-y-4">{records.map(rec => <div key={rec.id} className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center justify-between"><div><div className="font-bold text-white text-base">{rec.title}</div><div className="text-xs text-slate-400">{rec.game_title} • {rec.category}</div></div><div className="text-right"><div className="font-mono font-black text-amber-400 text-lg">{rec.score_value} {rec.score_unit}</div><span className="px-2 py-0.5 rounded-md bg-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-wider">VERIFIED PROOF</span></div></div>)}</div></div>}</div>
    </main><Footer />
  </div>;
}
