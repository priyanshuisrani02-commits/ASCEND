...
import { Swords, Trophy, Flame, UserCheck, ShieldCheck } from 'lucide-react';
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        <div className="glass-panel p-8 sm:p-12 rounded-3xl border border-violet-500/30 space-y-8 relative overflow-hidden mb-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
            <div><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{challenge.game_title || 'Game Challenge'}</span><h1 className="text-3xl sm:text-5xl font-black text-white mt-1">{challenge.title}</h1></div>
            <span className="px-3.5 py-1.5 rounded-lg bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-xs uppercase tracking-wider self-start sm:self-center">{challenge.difficulty} DIFFICULTY</span>
          </div>
