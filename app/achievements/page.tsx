...
export default function AchievementsPage() {
  ...
  useEffect(() => {
    Promise.all([getAchievements(), getGames()]).then(([aData, gData]) => {
    ...
    <div className="min-h-screen bg-[#090a0f] text-slate-100 flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-white">ACHIEVEMENT DIRECTORY</h1>
          <p className="text-sm text-slate-400 mt-2 max-w-2xl">
            Browse all competitive achievements, requirements, XP payouts, and dynamic global unlock rarities.
          </p>
        </div>

        {/* SEARCH & FILTERS */}
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4 mb-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-6 relative">
              ...
              <input
                type="text"
                placeholder="Search achievement title or requirements..."
                value={search}
        ...
        {filteredAchievements.length > 0 ? (
          ...
          <div className="glass-panel p-12 text-center rounded-3xl border border-slate-800 my-10">
            <Shield className="w-12 h-12 text-slate-600 mx-auto mb-3" />
            <h3 className="text-lg font-bold text-slate-300">NO ACHIEVEMENTS FOUND</h3>
            <p className="text-xs text-slate-500 mt-1">Try resetting your rarity or game filter options.</p>
          </div>
        )}
