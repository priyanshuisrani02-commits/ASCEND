...
import { Trophy, Crown, Search, Sparkles, Scroll, Filter } from 'lucide-react';
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        <div className="glass-panel p-8 sm:p-10 rounded-3xl border border-violet-500/40 bg-gradient-to-r from-violet-950/40 via-slate-950 to-indigo-950/40 relative overflow-hidden mb-12">
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="flex items-center gap-6"><div className="relative"><img src={profile?.avatar_url || ''} alt="Avatar" className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl object-cover border-2 border-violet-400" /><Crown className="w-6 h-6 text-amber-400 absolute -top-2 -right-2" /></div><div><div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-600/20 border border-violet-500/40 text-violet-300 text-xs font-bold uppercase tracking-widest mb-1"><Sparkles className="w-3.5 h-3.5" />Adventurer Quest Vault</div><h1 className="text-3xl sm:text-5xl font-black text-white">{profile?.username || 'Your Deeds'}</h1><p className="text-xs text-slate-400 font-mono mt-1">{profile?.ranking_points ?? 0} RP • LEVEL {profile?.level ?? 1}</p></div></div>
