        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          <div>
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-bold uppercase tracking-widest mb-2">
              ...
              <span>Tournament Scheduler</span>
            </div>
          ...
          <Button variant="primary" onClick={() => setShowModal(true)}>
            ...
            <span>SCHEDULE NEW CHALLENGE</span>
          </Button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900 border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <th className="py-4 px-6">XP & RP Payout</th>
                <th className="py-4 px-6">Entries</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800 text-sm">
              {challenges.map(ch => (
                <tr key={ch.id} className="hover:bg-slate-900/60">
                  <td className="py-4 px-6 font-bold text-white">
                    <div>{ch.title}</div>
                    <div className="text-xs text-slate-400 font-normal">{ch.description}</div>
                  </td>
                  <td className="py-4 px-6 font-mono text-slate-300">{ch.game_title || 'Game'}</td>
                  <td className="py-4 px-6 font-bold text-amber-400">{ch.difficulty}</td>
                  <td className="py-4 px-6 font-mono text-xs">
                    <span className="text-amber-400 font-bold">+{ch.xp_reward} XP</span> / <span className="text-violet-400 font-bold">+{ch.ranking_reward} RP</span>
                  </td>
                  <td className="py-4 px-6 font-mono font-bold text-slate-300">{ch.participant_count.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <div className="glass-panel w-full max-w-lg p-6 sm:p-8 rounded-3xl border border-violet-500/30">
              <h2 className="text-xl font-black text-white mb-4">SCHEDULE WEEKLY CHALLENGE</h2>
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">RP Reward</label>
                    <input
                      type="number"
                      required
                      value={rankingReward}
                      onChange={e => setRankingReward(e.target.value)}
                      className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-200 text-xs focus:border-violet-500 outline-none"
                    />
                  </div>
                </div>
