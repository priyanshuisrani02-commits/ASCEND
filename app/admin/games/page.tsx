...
import { Gamepad2, Plus, CheckCircle2 } from 'lucide-react';
...
export default function AdminGamesPage() {
  ...
  const [coverUrl, setCoverUrl] = useState('https://images.unsplash.com/photo-1542751371-adc38448a05e?w=600&auto=format&fit=crop&q=80');
    ...
    <div className="min-h-screen bg-[#090a0f] text-slate-100 flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          <div>
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-300 text-xs font-bold uppercase tracking-widest mb-2">
              ...
              {games.map(g => (
                <tr key={g.id} className="hover:bg-slate-900/60">
                  <td className="py-4 px-6 font-bold text-white flex items-center space-x-3">
                    <img src={g.cover_url} alt={g.title} className="w-9 h-12 rounded-lg object-cover" />
                    <span>{g.title}</span>
                  </td>
                  <td className="py-4 px-6 font-mono text-slate-300">{g.genre}</td>
                  <td className="py-4 px-6 font-mono text-violet-400 font-bold">{g.player_count.toLocaleString()}</td>
                  <td className="py-4 px-6 font-mono text-amber-400 font-bold">{g.achievement_count}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <div className="glass-panel w-full max-w-lg p-6 sm:p-8 rounded-3xl border border-violet-500/30">
              <h2 className="text-xl font-black text-white mb-4">ADD NEW GAME</h2>
              <form onSubmit={handleCreateGame} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Game Title</label>
                  <input
                    type="text"
                    required
                    value={title}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Genre</label>
                  <input
                    type="text"
                    required
                    value={genre}
                ...
                <div className="flex justify-end space-x-3 pt-2">
                  ...
                  <Button type="submit" variant="primary">Create Title</Button>
                </div>
              </form>
            </div>
          </div>
