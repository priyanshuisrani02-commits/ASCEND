...
import { Swords, Clock, Trophy, Flame, UserCheck, Sparkles } from 'lucide-react';
    ...
    <div className="glass-panel portal-glow p-6 rounded-3xl border border-violet-500/30 flex flex-col justify-between relative overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:border-violet-400/80 hover:shadow-[0_0_40px_rgba(124,58,237,0.35)]">
      ...
      <div>
        <div className="flex items-center justify-between mb-3 relative z-10">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center space-x-1.5"><Swords className="w-3.5 h-3.5 text-violet-400" /><span>{challenge.game_title || 'REALM TRIAL'}</span></span>
        ...
        <p className="text-xs text-slate-300 mt-2 leading-relaxed font-normal">{challenge.description}</p>
        <div className="mt-5 p-3.5 rounded-2xl bg-slate-950/90 border border-slate-800/80 flex items-center justify-between shadow-inner">
          <div className="flex items-center space-x-2 text-slate-400 text-xs font-bold"><Clock className="w-4 h-4 text-violet-400" /><span className="tracking-wider">PORTAL CLOSES IN:</span></div>
          <div className="font-mono text-sm font-black text-amber-300 tracking-widest">{timeLeft.days}d {timeLeft.hours}h {timeLeft.mins}m {timeLeft.secs}s</div>
        </div>
      </div>
      ...
      <div className="pt-5 mt-5 border-t border-slate-800/80 flex items-center justify-between relative z-10">
        ...
        {joined ? <span className="px-4 py-2 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-bold flex items-center space-x-1.5"><UserCheck className="w-4 h-4" /><span>ENTERED TRIAL</span></span> : <Button variant="primary" size="sm" onClick={handleJoin} className="px-5 py-2 font-bold tracking-wider"><Sparkles className="w-4 h-4 mr-1.5" /><span>ENTER TRIAL</span></Button>}
      </div>
    </div>
  );
};
